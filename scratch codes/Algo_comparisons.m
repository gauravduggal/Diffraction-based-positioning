...
%         end
        ...
        for fidx = 1:Nf
            ...
            [IPPA_nlos_mean_estimates_vec(:,fidx),IPPA_nlos_mean_residuals_normvec(fidx)] = mppa_floor(np_start, delta, a,r,nlos_flag, nlos_bias_mean(cidx,:,fidx),np);
            [IPPA_nlos_min_estimates_vec(:,fidx),IPPA_nlos_min_residuals_normvec(fidx)] = mppa_floor(np_start, delta, a,r,nlos_flag, nlos_bias_min(cidx,:,fidx),np);
            [IPPA_nlos_id_estimates_vec(:,fidx),IPPA_nlos_id_residuals_normvec(fidx)] = mppa_floor(np_start, delta, a,r,nlos_flag, zeros(7,1),np);        
            %NLS
            Niter = 100;
            upper_edge_x_coord = xn_est + wh/2;
            X1 = [upper_edge_x_coord;ynvec(1);0];
            X2 = [upper_edge_x_coord;ynvec(end);0];
            [NLS_estimates(:,fidx),NLS_residuals_normvec(fidx)] = fn_3D_estimator(r,a,np_start, Niter,wh, X1,X2,np);
